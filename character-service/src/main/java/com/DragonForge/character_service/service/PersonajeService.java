...
// Requisito WebClient: Consumir endpoint del microservicio RNG de dados externo
        RollResultDTO resultadoDados = webClient.get()
                ...
                .block(); // Bloqueo controlado para arquitectura secuencial académica
        // Asignar dinámicamente los puntos de vida retornados por el microservicio externo
        if (resultadoDados != null) {
            personaje.setPuntosGolpe(resultadoDados.getTotal());
        } else {
            personaje.setPuntosGolpe(15); // Fallback seguro
        }
